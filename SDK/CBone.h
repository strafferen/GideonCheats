...
{
public:
	UINTPTR GetMatrix(UINTPTR CCSPlayerPawn)
	{
		if (CCSPlayerPawn != NULL)
		{
			UINTPTR PlayerNode = Driver::RPM<UINTPTR>(CCSPlayerPawn + Offsets::m_pGameSceneNode);
			UINTPTR BoneMatrix = Driver::RPM<UINTPTR>(PlayerNode + Offsets::m_groundNormal);
			return BoneMatrix;
		}
		else
		{
	...
	BoneConnection_Struct Connections[16] = {
